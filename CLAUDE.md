...
Deploys are tag-driven: pushing a CalVer tag (e.g. `2026.8.0`) runs
`deploy-pages.yml`, which re-runs those checks and publishes `./dist` to GitHub
Pages. Pushing to `master` on its own does not deploy.

## Architecture

### Pure logic vs. DOM (the split to preserve)

- **`src/clocks.ts`** holds the pure, side-effect-free helpers: URL parsing
...
- **`src/main.ts`** is the DOM driver: reads `location.search`, builds one card
...
gradient. Card accents are reserved for the data-driven day/night bands, so keep
...
which is what keeps any number of cities from overflowing on any resolution
